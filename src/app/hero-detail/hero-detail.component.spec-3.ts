...
describe('Hero Detail Component', () => {
  ...
  it(`
    ...
    const input = null;
    expect(input.nativeElement.value).toBe('Fred Flinstone');
  }));
});
...
ℹ️ FYI: If you just want to execute a single test - then you can FOCUS on one by writing fit('') instead of it('') 
  ...
  🔶 https://angular.io/api/core/testing/ComponentFixture#whenStable                                         
     ...
     Using whenStable() also means we have to use the async(() => {}) wrapper for our test implementation itself.
     
     When you leave the call to async() out - the test probably continous to work - but this is only because we are lucky with timing - it could even be a false positive an we are not noticing it.
  ...
  ⬇⬇⬇⬇⬇⬇⬇⬇
  📌📌📌 There is also some information down below that we want to pin in your brain 🧠
  ⬇⬇⬇⬇⬇⬇⬇⬇
...
Solution:
  ...
  🗣 Why the heck do we need this whenStable stuff?!
  The answer lies within the ngModelDirective of Angular itself.
  
  Take a look at this line:
   - https://github.com/angular/angular/blob/master/packages/forms/src/directives/ng_model.ts#L336
  
  There is a little explanation there as well:
   - https://github.com/angular/angular/blob/master/packages/forms/src/directives/ng_model.ts#L46

  Boils down to me to: 
  - a Promise is used 
  - a Promise is put into the microtask of the browser - it's async !
  - a Promise that immediately resolves is no exception there - it's async ! 
